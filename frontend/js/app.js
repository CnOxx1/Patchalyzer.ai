...
$$(".stab").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    $$(".s-pane").forEach(p => p.classList.toggle("active", p.id === `s-${btn.dataset.s}`));
  });
});
...
$$(".preset").forEach(btn => {
  btn.addEventListener("click", () => {
    $("#settings-form").base_url.value = btn.dataset.url;
    $("#settings-form").model.value = btn.dataset.model;
  });
});

$("#reset-agent-prompt")?.addEventListener("click", () => {
...
$("#gepa-optimize")?.addEventListener("click", async () => {
  flushAgentPrompt();
  const id = $("#prompt-agent")?.value || currentPromptAgent;
  const btn = $("#gepa-optimize");
  const msg = $("#settings-msg");
  if (!id) return;
  btn.disabled = true;
...
function pollJob(jobId, switchToJobs = false) {
  ...
  pollTimer = setInterval(async () => {
    ...
    const pct = job.progress?.percent ?? (job.status === "completed" ? 100 : job.status === "failed" || job.status === "cancelled" ? 0 : 10);
    $("#progress-fill").style.width = pct + "%";
    $("#progress-text").textContent = job.progress?.message || job.status + (job.error ? ": " + job.error : "");
    $("#cancel-job")?.classList.toggle("hidden", !(job.status === "running" || job.status === "pending"));

    if (job.status === "completed" || job.status === "failed" || job.status === "cancelled") {
      clearInterval(pollTimer);
      if (switchToJobs) activateTab("jobs");
      else loadJobs();
      openJobModal(jobId);
    }
  }, 1500);
...
function bindJobRows(root) {
  root?.querySelectorAll(".job-item").forEach(item => {
    item.addEventListener("click", () => openJobModal(item.dataset.id));
  });
}
...
$("#refresh-research")?.addEventListener("click", loadResearchTab);
$("#research-job-select")?.addEventListener("change", () => refreshResearchBoard().catch(() => {}));
$("#research-open-job")?.addEventListener("click", () => {
  const id = $("#research-job-select")?.value;
  if (id) openJobModal(id, "research");
});
$("#research-start")?.addEventListener("click", async () => {
  const id = $("#research-job-select")?.value;
  const btn = $("#research-start");
  if (!id) {
    alert("请选择已完成的分析任务");
    return;
  }
  const runLlm = $("#research-run-llm")?.checked !== false;
  if (btn) btn.disabled = true;
  try {
    const r = await fetch(`${API}/jobs/${id}/research`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ run_llm: runLlm }),
    });
    const data = await r.json().catch(() => ({}));
    if (!r.ok) {
      alert(typeof data.detail === "string" ? data.detail : (data.detail && JSON.stringify(data.detail)) || "无法启动研究流程");
      if (btn) btn.disabled = false;
      return;
    }
    await refreshResearchBoard();
  } catch (err) {
...
$("#research-cancel")?.addEventListener("click", async () => {
  const id = $("#research-job-select")?.value;
  if (!id) return;
  await fetch(`${API}/jobs/${id}/research/cancel`, { method: "POST" });
  await refreshResearchBoard();
});
...
function stripJsonPrefix(text) {
  ...
  const fence = lead.match(/^```(?:json)?\s*/i);
  const body = fence ? lead.slice(fence[0].length) : lead.trimStart();
  if (!body.startsWith("{")) return s.trim();
  ...
  for (let i = start; i < s.length; i++) {
    ...
    if (c === "\"") inStr = true;
      ...
      depth--;
      if (depth === 0) {
        let rest = s.slice(i + 1).replace(/^\s*```\s*/, "");
        rest = rest.replace(/^(?:#+\s*)?(?:中文\s*)?(?:Markdown\s*)?(?:技术)?解读\s*$/im, "");
        return rest.trim();
      }
    }
  }
...
async function hydrateMarkdown(root) {
  ...
  try {
    if (!hiddenPanel) await renderMermaidIn(root);
    typesetMath(root);
  } finally {
...
function setRing(status, resizedCount) {
  const ring = $("#detect-ring");
  const fg = $("#ring-fg");
  const circ = 2 * Math.PI * 40;
  ring.classList.remove("ok", "err", "run");
  let offset = circ;
  if (status === "completed") {
    ring.classList.add("ok");
    offset = 0;
    $("#ring-value").textContent = String(resizedCount ?? 0);
    $("#ring-sub").textContent = "Δ funcs";
  } else if (status === "failed") {
    ring.classList.add("err");
    offset = circ * 0.25;
    $("#ring-value").textContent = "!";
    $("#ring-sub").textContent = "失败";
  } else {
    ring.classList.add("run");
    offset = circ * 0.55;
    $("#ring-value").textContent = "…";
    $("#ring-sub").textContent = statusLabel(status);
  }
...
function nodeDetailHtml(id, job) {
  ...
  const note = notes[GRAPH_NOTE_KEY[id]];
  ...
  const noteBlock = note
    ? `<p class="card-title">专家输出</p><pre class="note-pre">${esc(String(note).slice(0, 4000))}${String(note).length > 4000 ? "\n…[截断]" : ""}</pre>`
    : (id === "ReportWriter" && art.llm_report
      ? `<p class="card-title">报告预览</p><pre class="note-pre">${esc(art.llm_report.slice(0, 2500))}${art.llm_report.length > 2500 ? "\n…" : ""}</pre>`
      : (id === "DetectionAnalyst" && art.ioc_pack
        ? `<p class="card-title">IOC 摘要</p><pre class="note-pre">${esc(JSON.stringify({ cve: art.ioc_pack.cve, component: art.ioc_pack.component, identity: art.ioc_pack.identity, apis: art.ioc_pack.apis }, null, 2).slice(0, 2500))}</pre>`
        : (id === "ThreatIntelAnalyst" && art.threat_intel
          ? `<p class="card-title">公开情报</p><pre class="note-pre">${esc(JSON.stringify({ cve: art.threat_intel.cve, status: art.threat_intel.status, in_kev: art.threat_intel.in_kev, ransomware_campaign: art.threat_intel.ransomware_campaign, summary: art.threat_intel.summary }, null, 2).slice(0, 2500))}</pre>`
          : (id === "BypassAnalyst" && art.bypass_pack
            ? `<p class="card-title">绕过面摘要</p><pre class="note-pre">${esc(JSON.stringify({ verdict: art.bypass_pack.verdict, summary: art.bypass_pack.summary, findings: art.bypass_pack.findings }, null, 2).slice(0, 2500))}</pre>`
            : (id === "ResidualVulnAnalyst" && art.residual_pack
              ? `<p class="card-title">残留摘要</p><pre class="note-pre">${esc(JSON.stringify({ verdict: art.residual_pack.verdict, summary: art.residual_pack.summary, findings: art.residual_pack.findings }, null, 2).slice(0, 2500))}</pre>`
              : (id === "AliasSiteAnalyst" && art.alias_pack
                ? `<p class="card-title">调用点覆盖</p><pre class="note-pre">${esc(JSON.stringify({ verdict: art.alias_pack.verdict, summary: art.alias_pack.summary, findings: art.alias_pack.findings }, null, 2).slice(0, 2500))}</pre>`
                : (id === "FeatureOffAnalyst" && art.feature_off_pack
                  ? `<p class="card-title">Feature 关闭路径</p><pre class="note-pre">${esc(JSON.stringify({ verdict: art.feature_off_pack.verdict, summary: art.feature_off_pack.summary, findings: art.feature_off_pack.findings }, null, 2).slice(0, 2500))}</pre>`
                  : (id === "HuntPrep" && art.hunt_brief
                ? `<p class="card-title">狩猎简报</p><pre class="note-pre">${esc(JSON.stringify({ goal: art.hunt_brief.goal, high_priority: art.hunt_brief.high_priority, alias_sites: art.hunt_brief.alias_sites, feature_off_sites: art.hunt_brief.feature_off_sites, clone_sites: art.hunt_brief.clone_sites, cfg_gaps: art.hunt_brief.cfg_gaps, skip_windows: art.hunt_brief.skip_windows, patched_pattern: art.hunt_brief.patched_pattern, candidates: (art.hunt_brief.candidates || []).map(c => ({ name: c.name, why: c.why, priority: c.priority, missing_lock_vs_patch: c.missing_lock_vs_patch, missing_feature_vs_patch: c.missing_feature_vs_patch })) }, null, 2).slice(0, 2500))}</pre>`
                : ""))))))));
...
function renderHuntLabPanel(job) {
  ...
  const logHtml = logs.map(item => `<li><span class="huntlab-track">${esc(item.track)}</span> <code>${esc(item.tool || "")}</code> ${esc(JSON.stringify(item.args || {}))}<pre>${esc(item.result_preview || "")}</pre></li>`).join("");
...
async function loadResearchTab() {
  const sel = $("#research-job-select");
  const board = $("#research-board");
  const hint = $("#research-run-hint");
  if (!sel) return;
  ...
  sel.innerHTML = done.length
    ? done.map(j => `<option value="${esc(j.id)}">${esc(j.title || j.id)}</option>`).join("")
    : `<option value="">（没有已完成任务）</option>`;
  if (prev && [...sel.options].some(o => o.value === prev)) sel.value = prev;
  else if (currentJobId && [...sel.options].some(o => o.value === currentJobId)) sel.value = currentJobId;
  if (!done.length) {
    if (hint) hint.textContent = "请先在「上传分析」或「补丁列表」完成一次对照，再选任务启动。";
...
async function refreshResearchBoard() {
  const id = $("#research-job-select")?.value;
  const board = $("#research-board");
  const progEl = $("#research-progress");
  const hint = $("#research-run-hint");
  const cancelBtn = $("#research-cancel");
  if (!id) {
    if (board) board.innerHTML = "";
    return;
  }
  ...
  if (hint) {
    hint.textContent = running
      ? (job.research_lab_progress?.message || "研究流程运行中…")
      : (lab.status === "completed" ? "上次结果如下。可再次启动覆盖。" : "选好任务后点「启动研究流程」。关闭 LLM 时只跑表面图与打分。");
  }
  cancelBtn?.classList.toggle("hidden", !running);
  if (progEl) {
    if (running || job.research_lab_progress) {
      progEl.classList.remove("hidden");
      const pct = job.research_lab_progress?.percent ?? (lab.status === "completed" ? 100 : 0);
      progEl.innerHTML = `<span>${esc(job.research_lab_progress?.message || "研究流程运行中…")}</span><strong>${pct}%</strong>`;
    } else {
      progEl.classList.add("hidden");
    }
  }
  if (board) {
    board.innerHTML = renderResearchBody(job);
    hydrateMarkdown(board);
  }
  ensureResearchTabPoll(id, job);
}

function ensureResearchTabPoll(jobId, job) {
  ...
  researchLabTimer = setInterval(async () => {
    if ($("#tab-research")?.classList.contains("active") && $("#research-job-select")?.value === jobId) {
      await refreshResearchBoard();
    }
...
function ensureResearchPoll(jobId, job) {
  if (!researchLabRunning(job)) return;
  ensureResearchTabPoll(jobId, job);
}
...
function ensureIocSection(md, pack) {
  const section = iocSectionMarkdown(pack).replace(/\s+$/, "") + "\n";
  const text = String(md || "").replace(/\r\n/g, "\n");
  if (/^##\s*16\./m.test(text)) {
    return text.replace(/^##\s*16\.\s[\s\S]*?(?=\n##\s*\d+\.|\s*$)/, section + "\n");
  }
...
function ensureThreatSection(md, pack) {
  const section = threatSectionMarkdown(pack).replace(/\s+$/, "") + "\n";
  const text = String(md || "").replace(/\r\n/g, "\n");
  if (/^##\s*17\./m.test(text)) {
    return text.replace(/^##\s*17\.\s[\s\S]*?(?=\n##\s*\d+\.|\s*$)/, section + "\n");
  }
  return `${text.replace(/\s+$/, "")}\n\n${section}\n`;
}
...
function ensureNumberedSection(md, number, section) {
  ...
  const text = String(md || "").replace(/\r\n/g, "\n");
  const re = new RegExp(`^##\\s*${number}\\.\\s[\\s\\S]*?(?=\\n##\\s*\\d+\\.|\\s*$)`);
  if (new RegExp(`^##\\s*${number}\\.`, "m").test(text)) {
...
async function openJobModal(jobId, panelName) {
  ...
  $("#modal-job-id").textContent = job.id;
  $("#download-ioc").href = `${API}/jobs/${jobId}/ioc.json`;
  ...
  $("#download-cfg").href = `${API}/jobs/${jobId}/cfg_diff.html`;
  $("#download-verify").href = `${API}/jobs/${jobId}/verify.zip`;
  ...
  setRing(job.status, resized.length);
  $("#modal-sub").textContent = [
    `${job.old_label || "漏洞版"} → ${job.new_label || "修复版"}`,
    pr ? `${pr.old_version || ""} → ${pr.new_version || ""}` : null,
  ...
  const peOld = art.old_pe || {};
  const peNew = art.new_pe || {};
  const imps = importDiff(peOld, peNew);
  const impRows = imps.map(r => `<tr>
    <td><code>${esc(r.dll)}</code></td>
    <td>${r.old}</td><td>${r.neu}</td>
    <td class="${r.delta >= 0 ? "delta-pos" : "delta-neg"}">${r.delta >= 0 ? "+" : ""}${r.delta}</td>
  </tr>`).join("");
  const callRows = (art.disassembly || []).map(b => {
  ...
  $("#panel-summary").innerHTML = `
    ...
    <div class="stats">
      <div class="stat"><span>状态</span><strong>${statusLabel(job.status)}</strong></div>
      <div class="stat"><span>尺寸变化</span><strong>${resized.length}</strong></div>
      <div class="stat"><span>符号 + / −</span><strong>${(sym.symbols_added||[]).length} / ${(sym.symbols_removed||[]).length}</strong></div>
      <div class="stat"><span>Feature 组</span><strong>${features.length || (sym.feature_symbols_added||[]).length || 0}</strong></div>
    </div>
    ${pr ? `<p class="hint">自动匹配：${esc(pr.old_file)} ${esc(pr.old_version)} → ${esc(pr.new_version)}（KB${esc((pr.matched_kbs||[]).join(", KB"))}）</p>` : ""}
    ${chain.present ? `<p class="hint">漏洞链已就绪（${(chain.steps || []).length || "?"} 步）· 见「漏洞链」页签</p>` : `<p class="hint">尚未提取漏洞链。重新生成报告后可在「漏洞链」页签查看。</p>`}
    ${art.llm_error ? `<p class="err">${esc(art.llm_error)}</p>` : ""}
    ${job.error ? `<p class="err">${esc(job.error)}</p>` : ""}
    <p class="card-title">样本对照</p>
    <div class="ident-grid">
      ${[
        ...
        <article class="ident-card">
          ...
          <p class="hint">${esc(pe.machine || "—")} · ${pe.size ? fmtBytes(pe.size) : "—"} · ${esc(pe.timestamp_utc || "")}</p>
          <div class="ident-dl">
            ${hashLine("SHA256", pe.sha256)}
            ${hashLine("SHA1", pe.sha1)}
            ${hashLine("PDB", pdbGuid(pe))}
          </div>
        </article>`).join("")}
    </div>
    ${callRows ? `<p class="card-title">热点调用差</p>${table(["函数","Δ","新增调用","删除调用"], callRows)}` : ""}
    ${impRows ? `<p class="card-title" style="margin-top:1.2rem">导入表变化（按 DLL 计数）</p>${table(["DLL","Old","New","Δ"], impRows)}` : `<p class="hint">导入表 DLL 计数无变化。</p>`}
    ${notes.pe ? `<p class="card-title" style="margin-top:1.2rem">版本归因</p><div class="ioc-notes report-md md-compact">${mdHtml(notes.pe)}</div>` : ""}
    ${hotspotPickerHtml(art, job)}
  `;

  try {
    $("#panel-chain").innerHTML = renderChainPanel(art);
  } catch (err) {
    console.warn("chain panel failed", err);
    $("#panel-chain").innerHTML = `<section class="chain-page"><h3>函数逻辑漏洞链</h3><p class="err">绘制失败：${esc(err.message || String(err))}</p></section>`;
  }
  ...
  await hydrateMarkdown($("#panel-chain"));
  await hydrateMarkdown($("#panel-ioc"));
  await hydrateMarkdown($("#panel-threat"));
  await hydrateMarkdown($("#panel-bypass"));
  await hydrateMarkdown($("#panel-residual"));
  await hydrateMarkdown($("#panel-huntlab"));
  await hydrateMarkdown($("#panel-control"));
  await hydrateMarkdown($("#panel-summary"));
  await hydrateMarkdown($("#panel-feature"));

  const report = art.llm_report || "";
  const exportMd = reportMarkdown();
  const exportBtn = $("#export-report");
  if (exportBtn) exportBtn.disabled = !report && !exportMd.includes("## 16.");
  renderExportSectionPicker();

  activatePanel(panelName || (job.status === "completed" ? "summary" : "community"));
  ...
  if (job.status === "running" || job.status === "pending") {
    ...
    pollTimer = setInterval(async () => {
      ...
      if (currentJobId !== jobId) return;
      paintCommunity(j);
      setRing(j.status, ((j.result?.artifacts?.symbol_diff || {}).functions_resized || []).length);
...
$("#modal-close").addEventListener("click", closeJobModal);
document.addEventListener("keydown", e => {
  if (e.key !== "Escape") return;
  if (!$("#settings-drawer").classList.contains("hidden")) {
    closeSettings();
    return;
  }
  if ($("#job-modal").classList.contains("hidden")) return;
  if (graphSelectedId) {
    closeGraphPop();
    return;
  }
...
$("#panel-community").addEventListener("click", e => {
  const jump = e.target.closest("[data-goto-panel]");
  if (jump) {
    activatePanel(jump.dataset.gotoPanel);
    return;
  ...
  if (node) openGraphPop(node.dataset.node, { toggle: true });
});
$("#panel-chain")?.addEventListener("click", e => {
  const jump = e.target.closest("[data-goto-panel]");
  if (jump) activatePanel(jump.dataset.gotoPanel);
});
$("#panel-ioc")?.addEventListener("click", e => {
  const jump = e.target.closest("[data-goto-panel]");
  if (jump) activatePanel(jump.dataset.gotoPanel);
});
$("#panel-threat")?.addEventListener("click", async e => {
  const jump = e.target.closest("[data-goto-panel]");
  if (jump) {
    activatePanel(jump.dataset.gotoPanel);
    return;
  ...
  if (!refresh || !currentJobId) return;
...
$("#panel-bypass")?.addEventListener("click", e => {
  const jump = e.target.closest("[data-goto-panel]");
  if (jump) activatePanel(jump.dataset.gotoPanel);
});
$("#panel-residual")?.addEventListener("click", e => {
  const jump = e.target.closest("[data-goto-panel]");
  if (jump) activatePanel(jump.dataset.gotoPanel);
});
...
$("#panel-summary")?.addEventListener("click", e => {
  const jump = e.target.closest("[data-goto-panel]");
  if (jump) activatePanel(jump.dataset.gotoPanel);
});
$("#copy-job-id")?.addEventListener("click", async () => {
  ...
  try {
    ...
    $("#copy-job-id").textContent = "已复制";
    setTimeout(() => { $("#copy-job-id").textContent = "复制"; }, 1200);
  } catch { /* ignore */ }
});
...
$("#regen-report").addEventListener("click", async () => {
  if (!currentJobId) return;
  $("#regen-report").disabled = true;
  try {
    ...
    $("#regen-report").disabled = false;
  }
});

$("#export-report")?.addEventListener("click", e => {
  e.stopPropagation();
...
$("#export-menu")?.addEventListener("click", async e => {
  ...
  e.stopPropagation();
  let ok = false;
  if (item.dataset.export === "pdf") ok = await exportReportPdf();
  else if (item.dataset.export === "html") ok = await exportReportHtml();
  else if (item.dataset.export === "md") ok = await exportReportMarkdown();
  if (ok) $("#export-menu").classList.add("hidden");
});
$("#export-sections")?.addEventListener("change", () => persistExportSections(selectedExportSections()));
document.addEventListener("click", e => {
...
loadSettings();
{
  const saved = localStorage.getItem("patchalyzer.tab") || "home";
  activateTab(["home", "analyze", "patch", "jobs"].includes(saved) ? saved : "home");
}
